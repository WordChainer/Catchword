    ...
    function search(e) {
        ...
        $.post('/search', { keyword, length }).done(words => {
            words = words.map(word => {
                ...
                if (needFilter) {
                    marks.push('<i class="fas fa-ban mr-1 text-danger"></i>');
                }

                if (isHanbang33) {
                    marks.push('<i class="fas fa-skull mr-1 text-success"></i>');
                }

                if (isHanbang32) {
                    marks.push('<i class="fas fa-skull mr-1 text-primary"></i>');
                }

                let hasMarks = marks.length > 0;
                let marksHtml = hasMarks ? `<div style="font-size: 15px;">${marks.join('')}</div>` : '';
