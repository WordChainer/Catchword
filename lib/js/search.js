    ...
    function search(e) {
        ...
        $.post('/search', { keyword, length }).done(words => {
            words = words.map(({ value, date, user, isHidden }) => {
                return `<tr class="${['', 'bg-danger'][+isHidden]}"><td>${value}</td><td>${user.nickname}</td><td><span data-toggle="tooltip" data-html="true" title="${dayjs(date).format('YYYY년 MM월 DD일<br>HH시 mm분 ss초')}">${date.slice(0, 10)}</span></td></tr>`;
            });
